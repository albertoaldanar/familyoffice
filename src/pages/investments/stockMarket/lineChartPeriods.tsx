...
export const ResultsChart: React.FC<ResultsChartProps> = (props) => {
  const { currency, year, wealthBalancebyPeriod } = props;

  // Extract months from the first available data set
  const months = wealthBalancebyPeriod[0]?.values
    .filter((v) => v.year === year)
    .map((v) => v.month) || [];

  // Prepare the series by filtering data for each item (e.g., banks)
  const series = wealthBalancebyPeriod.map((item) => ({
    name: item.name,
    data: item.values
      .filter((v) => v.year === year)
      .map((v) => {
