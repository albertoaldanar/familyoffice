export const prestamos = [
  {
    id: 1,
    deudor: "Solare SA de CV",
    tipo: "Prestamo a tercero",
    concepto: "Prestamo a DA Solare",
    monto: "2,000,000.00",
    notificationTo: { id: '23', name: 'Miembro 1 (Padre)', type: 'Family'},
    country: 'México',
    porPagar: "1,500,000.00",
    creditor: {
      name: 'Empresa financiera SA de CV',
      type: 'Empresa Familiar', 
      linkedItemId: 24
    },
    debtor: {
      name: 'Solare SA de CV', 
      phone: '667104822', 
      email: 'contacto@dasolare.com',
      type: null,
      linkedItemId: null
    },
    contrato: "https://googledrive.com/erg3440398f4fmv",
    fechaDeContratacion: "22/11/2022",
    fechaVencimiento: "01/12/2027",
    frecuenciaDePago: "Mensual",
    pagado: "500,000",
    moneda: "MXN",
    interes: "15",
    proxpago: "14 dias",
    proxCobro: "10/10/2024",
    pagos: [
      {
        id: 2,
        anio: "2024",
        limitePago: "05/05/2024",
        mes: "Mayo",
        monto: "250,000.00",
        proximoPago: "",
        fechaDePago: "05/05/2024",
        proximoPago: "",
        comprobantePago: "https://googledrive.com/erg3440398f4fmv",
      },
      {
        id: 3,
        anio: "2024",
        limitePago: "05/06/2024",
        mes: "Junio",
        monto: "250,000.00",
        proximoPago: "",
        fechaDePago: "",
        comprobantePago: "",
      },
    ],
  },
  {
    id: 2,
    deudor: "Miembro 4 (Hija)",
    proxCobro: "10/10/2024",
    notificationTo: { id: '24', type: 'Provider', name: 'Contadora 1'},
    country: 'México',
    creditor: {
      name: 'Empresa financiera SA de CV',
      type: 'Empresa Familiar', 
      linkedItemId: 24
    },
    debtor: {
      name: 'Miembro 4 (Hija)', 
      type: 'Miembro de la familia', 
      linkedItemId: 13
    },
    tipo: "Prestamo intrafamiliar",
    concepto: "Prestamo Miembro 4 (Hija)",
    monto: "10,000.00",
    porPagar: "9,000.00",
    contrato: "https://googledrive.com/erg3440398f4fmv",
    fechaDeContratacion: "22/11/2022",
    fechaVencimiento: "01/12/2027",
    frecuenciaDePago: "Mensual",
    pagado: "1000.00",
    moneda: "MXN",
    interes: "0",
    proxpago: "14 dias",
    pagos: [
      {
        id: 2,
        anio: "2023",
        limitePago: "12/04/2024",
        mes: "Abril",
        monto: "300.00",
        proximoPago: "",
        fechaDePago: "16/04/2024",
        proximoPago: "",
        comprobantePago: "https://googledrive.com/erg3440398f4fmv",
      },
      {
        id: 3,
        anio: "2024",
        limitePago: "12/05/2024",
        mes: "Mayo",
        monto: "700.00",
        proximoPago: "12/06/2024",
        fechaDePago: "16/05/2024",
        comprobantePago: "https://googledrive.com/erg3440398f4fmv",
      },
    ],
  },
  {
    id: 33,
    deudor: "Raul Quintero Flores",
    proxCobro: "10/10/2024",
    country: 'México',
    creditor: {
      name: 'Miembro 1 (Padre)',
      type: 'Miembro de la familia', 
      linkedItemId: 23
    },
    notificationTo: { id: '24', type: 'Provider', name: 'Contadora 1'},
    debtor: {
      name: 'Raul Quintero Flores', 
      phone: '667104822', 
      email: 'raulquinterof@gmail.com',
      type: null, 
      linkedItemId: null
    },
    tipo: "Deuda Capital privado",
    concepto: "Prestamo contructora ABD",
    monto: "10,000,000.00 ",
    porPagar: "10,000,000.00",
    contrato: "https://googledrive.com/erg3440398f4fmv",
    fechaDeContratacion: "22/11/2022",
    fechaVencimiento: "01/12/2027",
    frecuenciaDePago: "Mensual",
    pagado: "1000.00",
    moneda: "MXN",
    interes: "9",
    proxpago: "30 dias",
    pagos: [
      {
        id: 2,
        anio: "2023",
        limitePago: "12/04/2024",
        mes: "Abril",
        monto: "300.00",
        proximoPago: "",
        fechaDePago: "16/04/2024",
        proximoPago: "",
        comprobantePago: "https://googledrive.com/erg3440398f4fmv",
      },
      {
        id: 3,
        anio: "2024",
        limitePago: "12/05/2024",
        mes: "Mayo",
        monto: "700.00",
        proximoPago: "12/06/2024",
        fechaDePago: "16/05/2024",
        comprobantePago: "https://googledrive.com/erg3440398f4fmv",
      },
    ],
  },


  // {
  //   id: 24,
  //   fundName: "",
  //   investment: "1000000",
  //   loanId: 5,
  //   investmentReturns: [],
  //   privateEquityType: "Directo",
  //   directType: 'Deuda',
  //   investmentName: "Prestamo a constructor ABD",
  //   preMoneyValue: "",
  //   postMoneyValue: "",
  //   equityPercentage: "",
  //   currentValue: "",
  //   tir: "",
  //   voucher: "",
  //   companyStage: "",
  //   actaConstitutiva: "",
  //   actaAsamblea: "",
  //   financialStatements: [],
  //   containedIntrusts: [],
  //   currency: "USD",
  //   investmentYear: "",
  //   fundType: "",
  //   industry: "",
  //   country: "México",
  //   owners: [
  //     {
  //       type: "family",
  //       pct: "100",
  //       coreId: 43,
  //       name: "Francisco Aldana Fariñas",
  //     },
  //   ],
  // },
];

export const dividends = [
  {
    fechaCobro: "12/02/2024",
    sePagoA: "Miembro 1 (Padre)",
    empresa: "Empresa Agricola SA de CV",
    monto: "$9,000,000.00 MXN",
    proxCobro: "3 meses",
  },
  {
    fechaCobro: "12/02/2024",
    sePagoA: "Miembro 4 (Hija)",
    empresa: "Empresa financiera SA de CV",
    monto: "$2,000,000.00 MXN",
    proxCobro: "6 meses",
  },
];

export const arrendamientos = [
  {
    id: 21,
    tipo: "Inmobiliario",
    arrendatario: "John Williams",
    moneda: 'MXN',
    linkedItemId: 24,
    plazo: "1 año",
    country: 'México',
    concepto: "Departamento Puerta Cabo Village",
    monto: "34,000.00",
    contrato: "https://googledrive.com/erg3440398f4fmv",
    proxpago: "8 dias",
    tenantPhone: '667194849',
    proxCobro: "22/09/2024",
    frecuenciaDePago: "Mensual",
    notificationTo: { id: '43', name: 'Miembro 2 (Madre)', type: 'Family' },
    pagos: [
      {
        id: 94,
        anio: "2023",
        vigenciaDel: "20/12/2022",
        vigenciaAl: "20/12/2070",
        limitePago: "12/02/2022",
        proximoPago: "",
        mes: "Enero",
        fechaDePago: "12/02/2022",
        monto: "34,000.00",
        comprobantePago: "https://googledrive.com/erg3440398f4fmv",
      },
      {
        id: 95,
        anio: "2023",
        vigenciaDel: "20/12/2022",
        vigenciaAl: "20/12/2070",
        limitePago: "12/02/2023",
        fechaDePago: "12/02/2022",
        proximoPago: "09/06/2024",
        mes: "Febrero",
        monto: "34,000.00",
        comprobantePago: "https://googledrive.com/erg3440398f4fmv",
      },
    ],
  },
  {
    id: 8,
    tipo: "Inmobiliario",
    arrendatario: "Empresa Agricola SA de CV",
    notificationTo:  { id: '43', name: 'Miembro 2 (Madre)', type: 'Family' },
    linkedItemId: 42,
    plazo: "1 año",
    country: 'México',
    proxCobro: "22/09/2024",
    tenantPhone: '669194849',
    concepto: "Renta oficinas Mutualismo",
    monto: "45,000.00",
    contrato: "",
    proxpago: "1 dia",
    moneda: "MXN",
    frecuenciaDePago: "Mensual",
    pagos: [
      {
        id: 50,
        anio: "2023",
        vigenciaDel: "20/12/2022",
        vigenciaAl: "20/12/2070",
        limitePago: "12/02/2022",
        fechaDePago: "12/02/2022",
        proximoPago: "",
        mes: "Enero",
        monto: "45,000.00",
        comprobantePago: "https://googledrive.com/erg3440398f4fmv",
      },
      {
        id: 51,
        anio: "2024",
        vigenciaDel: "20/12/2022",
        vigenciaAl: "20/12/2070",
        limitePago: "12/02/2023",
        fechaDePago: "",
        proximoPago: "",
        mes: "Febrero",
        monto: "45,000.00",
        comprobantePago: "",
      },
    ],
  },
  {
    id: 99,
    tipo: "Otro",
    arrendatario: "Claudia Jimenez",
    linkedItemId: null,
    plazo: "1 año",
    country: 'México',
    proxCobro: "22/09/2024",
    tenantPhone: '669194849',
    concepto: "Renta de articulos",
    monto: "12,000.00",
    contrato: "",
    proxpago: "1 dia",
    moneda: "MXN",
    frecuenciaDePago: "Mensual",
    notificationTo: { id: '43', name: 'Miembro 2 (Madre)', type: 'Family' },
    pagos: [
      {
        id: 50,
        anio: "2023",
        vigenciaDel: "20/12/2022",
        vigenciaAl: "20/12/2070",
        limitePago: "12/02/2022",
        fechaDePago: "12/02/2022",
        proximoPago: "",
        mes: "Enero",
        monto: "45,000.00",
        comprobantePago: "https://googledrive.com/erg3440398f4fmv",
      },
    ],
  },
];
